pipeline {
    ...
    stages {
        ...
        stage('Build & Push Dockerfile.') {
            ...
            sh 'docker build .  -t  si3mshady/blogsite-fe:3' 
            sh 'docker login -u $USERNAME -p $PASSWORD'       
            sh 'docker push si3mshady/blogsite-fe:3'  
    ...
    stage('Scan Image.') {
            steps {
                ...
                -v $PWD:/project  snyk/snyk-cli:docker test --docker \
                  si3mshady/blogsite-fe:3  | tee results.scan                   
    ...
    stage('Test Scan Report for High Vuln Count') {
           ...
           sh '''count=$(cat results.scan | grep -i high | wc -l);
                 ...
                 if [ $result = PASS ]; then  docker save si3mshady/blogsite-fe:3 > frontend.tar ; else echo "FAIL"; fi;
